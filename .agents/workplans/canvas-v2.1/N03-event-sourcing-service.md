# N03 — Canvas Event Sourcing、Fold、CanvasService 与原子提交

> 项目：`zhangjaky71-stack/deepseek-harness`  
> 基线：Canvas / Media Workflow V2.1 Production Hardening  
> 文档性质：工程实施节点文档  
> 使用方式：后续可以直接引用节点编号进行“实施 / Code Review / 验收 / 修复”。  
> 总原则：具体 TypeScript API 签名以实施时仓库当前源码为准；职责边界、状态不变量和验收条件以本节点文档为准。

## 1. 节点目标

建立 Canvas 唯一 Host 写入口和 Session durable authority，完成 CAS、原子 operation、cache/replay 一致性。

## 2. 前置依赖

`N01, N02`

依赖节点未验收时，不应把本节点公开 API 视为稳定。

## 3. 本节点范围

- `canvas/change` 事件。
- 完整 post-change CanvasSnapshot 事件策略。
- foldCanvas / applyCanvasEvent。
- CanvasService cache/sync/prepareMutation/expectCurrent/commit。
- Workflow operations 原子应用。
- get/create/replace/edit/selectOutput/clear 等不涉及执行后端的 mutation。

## 4. 明确不在本节点处理

- 不越级实现尚未到达的后续 Provider/UI/治理能力，除非为编译所需的最小 seam。
- 不改变 V2.1 已冻结的核心不变量。
- 不通过临时 Browser state、直接 Provider 调用或 Session 私有 hack 绕过前置架构。

## 5. 预计代码位置

- `packages/canvas/canvas/src/index.ts`
- `packages/canvas/canvas/src/fold.ts`
- `packages/canvas/canvas/src/runtime.ts`
- `packages/canvas/canvas/src/domain.ts`
- `packages/canvas/canvas/tests/service.test.ts`
- `packages/canvas/canvas/tests/fold.test.ts`

实际开始实施时必须再次读取目标目录附近的 `AGENTS.md`，代码位置可依仓库当前结构小幅调整。

## 6. 核心接口 / 行为契约

事件：

```ts
interface CanvasChange {
  kind: 'canvas/change'
  version: number
  operation: CanvasOperation
  canvas: CanvasSnapshot | null
  meta: CanvasChangeMeta
}
```

原子编辑：

```text
clone current workflow
→ apply operations[]
→ validate whole result
→ increment workflowRevision once
→ append one canvas/change
```

任何一步失败，不得发布部分状态。

## 7. 实施步骤

1. 声明 `SessionEventMap['canvas/change']`。
2. 实现纯 fold，避免依赖 Service/ctx。
3. 实现 CanvasService runtime cache，与 Session seq 同步。
4. 实现 WorkflowRef CAS；stale 返回稳定 `CANVAS_STALE_WORKFLOW_REVISION`。
5. 实现 operations[] 内存 transaction。
6. commit 只在所有验证成功后 append Session。
7. 缓存只能优化，cold replay 必须得到相同 state。

## 8. 工程约束

- 所有 durable state 只在 commit point 发布。
- 产品可见 plugin 必须有符合仓库要求的 REAL composition coverage。
- package 行为变化同步更新 README/JSDoc。
- `src/types.ts` 保持 types-only；测试放 package-level `tests/`。
- 新增 package 必须提供 `./invariant` 并正确接 aggregate/build 配置。
- Registry/listener/subscription 必须证明 disposal/HMR 安全。

## 9. 测试要求

- [ ] cold replay 与 live state 完全相等。
- [ ] operations 中第 4 项失败时前 3 项不落盘。
- [ ] stale revision 被拒绝。
- [ ] runRevision 改变不使 workflow edit stale。
- [ ] clear 后 projection source state 为 null。
- [ ] 重复读取 cache 不改变 durable state。

## 10. 验收标准

- [ ] Agent/Browser 未来都只能通过 CanvasService 写业务状态。
- [ ] Session replay 是唯一恢复方式。
- [ ] 不存在 partial mutation。
- [ ] commit point 符合仓库 publish-state 规则。

## 11. Definition of Done

- [ ] 代码通过 typecheck/lint/build（按仓库对应命令）。
- [ ] 本节点单元测试通过。
- [ ] 必要 integration / REAL composition 测试通过。
- [ ] README/JSDoc 与公开行为一致。
- [ ] 没有未说明的架构偏差。
- [ ] 提交/PR 描述包含测试证据与剩余限制。

## 12. 风险与禁止项

- 为了 UI 方便直接 append node-level event；禁止作为 Domain authority。

## 13. 验收时应输出的结果

后续如果用户要求“验收本节点”，应至少输出：

1. 实际修改文件清单。
2. 关键接口与设计是否符合本节点契约。
3. 测试命令与结果。
4. REAL composition/E2E 证据（如适用）。
5. 未解决问题及严重度。
6. `ACCEPTED / ACCEPTED WITH FOLLOW-UP / REJECTED` 结论。

## 14. 实施指令示例

后续可以直接说：`实施 N03`、`检查 N03`、`验收 N03` 或 `修复 N03 验收问题`。
