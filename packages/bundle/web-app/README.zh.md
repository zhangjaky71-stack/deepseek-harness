...
动态名录还会挂载 Session-native [`ui-canvas`](../../client/ui-canvas/README.md)。Minimal 与 Editor Canvas body 消费 Session Projection，而常驻 Composer 仍由 `ui-conversation` 持有，因此切换 Canvas 模式不会移除 Prompt 输入。
...
`ui-canvas` 仅属于 Browser presentation，不贡献任何模型可见 Prompt 内容；其状态来自 Session Projection。
...
- **Canvas execution control 尚未接入真实执行**：当前 Canvas View 会按状态正确渲染 Run/Retry/Cancel control，但真实 Media execution 与 cancellation 仍属于后续 Host capability。
