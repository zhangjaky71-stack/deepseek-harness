...
The dynamic roster also mounts session-native [`ui-canvas`](../../client/ui-canvas/README.md). Minimal and Editor Canvas bodies consume Session Projection while the resident composer remains owned by `ui-conversation`, so changing Canvas mode never removes prompt entry.
...
`ui-canvas` is browser presentation only and contributes no model-visible prompt content; its state comes from Session Projection.
...
- **Canvas execution controls are not live yet** — the current Canvas view renders state-correct Run/Retry/Cancel controls, while real media execution and cancellation remain later Host capabilities.
