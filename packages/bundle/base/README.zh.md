...
基础名录还会挂载 Session-native Canvas Host authority、其 Deployment Feature Policy、`@deepseek-ai/dsh-media-workflow` 与 V1 语义 Media Node built-ins。因此 Browser Remote 与后续 Canvas consumer 会在所有 shipped profile 中解析到同一个 `ctx.canvas` authority、`ctx.canvasFeatures` deployment truth 和 effect-scoped `ctx.mediaNodes` catalog；Catalog metadata 始终是 process-local 数据，绝不会成为 Session state。
...
Canvas Feature Policy 与 Media Node Registry metadata 同样不会直接贡献 Prompt Text。
