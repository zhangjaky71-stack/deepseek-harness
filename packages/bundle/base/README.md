...
The shared dsh core as a profile bundle: [`cordis.patch.yml`](cordis.patch.yml) inserts every base plugin row — model adapters, the shared [`agent-default-model`](../../core/agent-default-model/README.md) selection, tools, persistence, policy, settings/credentials, telemetry, the session-native Canvas Host authority plus its deployment feature policy and semantic media-node registry, and host-level subagent providers — over the empty profile root, as the first layer of every profile's `dsh.profile.bundles` list. Canvas is mounted here together with `@deepseek-ai/dsh-canvas/feature-service`, `@deepseek-ai/dsh-media-workflow`, and its V1 built-ins, so Browser Remotes and later Validator/Editor/Agent/Executor consumers resolve the same `ctx.canvas` authority, `ctx.canvasFeatures` deployment truth, and `ctx.mediaNodes` definition catalog in every shipped profile. The feature row owns validated deployment defaults; the node catalog is process-local metadata rebuilt from effect-scoped plugin registrations and never becomes Session state. Later bundle layers (e.g. [`dsh-web-app`](../web-app/README.md)) and the user's profile `cordis.patch.yml` override rows by id; a patch replaces a row's whole `config`, so mode-specific values live in mode bundles, not here. The package has no runtime API; the profile composer resolves the patch through the `dsh.bundle.patch` manifest field, never through code.
...
Indirectly, through the inserted rows: this bundle selects the shipped persona-less prompt base, tool set, and DeepSeek adapter that mode bundles specialize, and contributes no model-visible text of its own. Canvas feature policy and media-node registry metadata likewise contribute no prompt text directly.
