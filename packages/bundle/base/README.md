...
The base roster also mounts the session-native Canvas Host authority, its deployment feature policy, `@deepseek-ai/dsh-media-workflow`, and the V1 semantic media-node built-ins. Browser Remotes and later Canvas consumers therefore resolve one `ctx.canvas` authority, `ctx.canvasFeatures` deployment truth, and effect-scoped `ctx.mediaNodes` catalog in every shipped profile; catalog metadata remains process-local and never becomes Session state.
...
Canvas feature policy and media-node registry metadata likewise contribute no prompt text directly.
