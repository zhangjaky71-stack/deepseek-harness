/** Static media-workflow validation over the active node-definition registry. */
...
/** Stable validation rejection carrying all blocking diagnostics. */
...
function structuralIndex(workflow: MediaWorkflow, diagnostics: MediaWorkflowDiagnostic[]): StructuralIndex {
  ...
  for (const node of workflow.nodes) {
    if (nodes.has(node.id)) {
      diagnostics.push({ severity: 'error', code: 'MEDIA_WORKFLOW_DUPLICATE_NODE_ID', message: `Workflow contains duplicate node id ${node.id}`, nodeId: node.id })
      continue
    }
  ...
  for (const edge of workflow.edges) {
    if (edgeIds.has(edge.id)) {
      diagnostics.push({ severity: 'error', code: 'MEDIA_WORKFLOW_DUPLICATE_EDGE_ID', message: `Workflow contains duplicate edge id ${edge.id}`, edgeId: edge.id })
      continue
    }
    edgeIds.add(edge.id)
    let valid = true
    if (!nodes.has(edge.sourceNodeId)) {
      diagnostics.push({ severity: 'error', code: 'MEDIA_WORKFLOW_UNKNOWN_SOURCE_NODE', message: `Edge ${edge.id} source node ${edge.sourceNodeId} does not exist`, edgeId: edge.id, nodeId: edge.sourceNodeId })
      valid = false
    }
    if (!nodes.has(edge.targetNodeId)) {
      diagnostics.push({ severity: 'error', code: 'MEDIA_WORKFLOW_UNKNOWN_TARGET_NODE', message: `Edge ${edge.id} target node ${edge.targetNodeId} does not exist`, edgeId: edge.id, nodeId: edge.targetNodeId })
      valid = false
    }
...
function validatePorts(
  ...
  for (const edge of edges) {
    ...
    if (sourcePort === undefined) {
      diagnostics.push({ severity: 'error', code: 'MEDIA_WORKFLOW_UNKNOWN_SOURCE_PORT', message: `Edge ${edge.id} references unknown source port ${edge.sourcePort} on ${edge.sourceNodeId}`, edgeId: edge.id, nodeId: edge.sourceNodeId, port: edge.sourcePort })
    }
    if (targetPort === undefined) {
      diagnostics.push({ severity: 'error', code: 'MEDIA_WORKFLOW_UNKNOWN_TARGET_PORT', message: `Edge ${edge.id} references unknown target port ${edge.targetPort} on ${edge.targetNodeId}`, edgeId: edge.id, nodeId: edge.targetNodeId, port: edge.targetPort })
    }
    if (sourcePort === undefined || targetPort === undefined) continue
    if (sourcePort.type !== targetPort.type) {
      diagnostics.push({ severity: 'error', code: 'MEDIA_WORKFLOW_PORT_TYPE_MISMATCH', message: `Edge ${edge.id} connects ${sourcePort.type} to ${targetPort.type}`, edgeId: edge.id, nodeId: edge.targetNodeId, port: edge.targetPort })
      continue
    }
    ...
    if (count > 1 && targetPort.multiple !== true) {
      diagnostics.push({ severity: 'error', code: 'MEDIA_WORKFLOW_INPUT_MULTIPLICITY', message: `Node ${edge.targetNodeId} input ${edge.targetPort} accepts only one edge`, edgeId: edge.id, nodeId: edge.targetNodeId, port: edge.targetPort })
    }
  }
  ...
  for (const node of nodes.values()) {
    ...
    for (const input of definition.inputs) {
      ...
      diagnostics.push({ severity: 'error', code: 'MEDIA_WORKFLOW_MISSING_REQUIRED_INPUT', message: `Node ${node.id} is missing required input ${input.name}`, nodeId: node.id, port: input.name })
    }
  }
}
...
function validateOutputsAndReachability(
  ...
  if (workflow.outputNodeIds.length === 0) {
    diagnostics.push({ severity: 'error', code: 'MEDIA_WORKFLOW_NO_OUTPUT', message: 'Workflow must declare at least one output node before execution' })
    return
  }
  ...
  for (const node of nodes.values()) {
    ...
    diagnostics.push({ severity: 'warning', code: 'MEDIA_WORKFLOW_UNREACHABLE_NODE', message: `Node ${node.id} does not contribute to a declared workflow output`, nodeId: node.id })
  }
}
 ...
 * @returns diagnostics plus a node-id-stable topological order when acyclic.
 */
export function validateMediaWorkflow(workflow: MediaWorkflow, registry: MediaNodeRegistry): MediaWorkflowValidationResult {
  ...
  return { valid: diagnostics.every(item => item.severity !== 'error'), diagnostics, topologicalNodeIds }
}

/**
 * Validate and materialize the exact definition map needed by scheduler/executor code.
