...
function existingNodes(workflow: ValidatedMediaWorkflow): Set<WorkflowNodeId> {
...
function assertTargets(workflow: ValidatedMediaWorkflow, selection: MediaWorkflowExecutionSelection): readonly WorkflowNodeId[] {
  ...
  if (selection.mode !== 'all' && unique.length === 0) {
    diagnostics.push({ severity: 'error', code: 'MEDIA_WORKFLOW_INVALID_PARTIAL_TARGET', message: `Partial execution mode ${selection.mode} requires at least one node` })
  }
  const existing = existingNodes(workflow)
  for (const nodeId of unique) {
    if (existing.has(nodeId)) continue
    diagnostics.push({ severity: 'error', code: 'MEDIA_WORKFLOW_INVALID_PARTIAL_TARGET', message: `Partial execution node ${nodeId} does not exist`, nodeId })
  }
...
function scheduledSet(
  ...
  if (selection.mode === 'downstream') for (const root of roots) descendants.delete(root)
  return descendants
}
...
function assertPartialSupport(
  ...
  for (const nodeId of scheduled) {
    ...
    diagnostics.push({ severity: 'error', code: 'MEDIA_WORKFLOW_PARTIAL_RUN_UNSUPPORTED', message: `Node ${nodeId} does not support partial execution`, nodeId })
  }
  if (diagnostics.length > 0) throw new MediaWorkflowValidationError(diagnostics)
}
...
function boundaries(workflow: ValidatedMediaWorkflow, scheduled: ReadonlySet<WorkflowNodeId>): readonly MediaWorkflowBoundaryInput[] {
  return workflow.workflow.edges
    ...
    .map(edge => ({ edgeId: edge.id, sourceNodeId: edge.sourceNodeId, sourcePort: edge.sourcePort, targetNodeId: edge.targetNodeId, targetPort: edge.targetPort }))
}

/**
 * Produce a deterministic execution scope over an already validated DAG.
 * `selected` includes required upstream ancestors; `from-node` includes the seed and descendants; `downstream` excludes the seed nodes.
 * @param workflow - validated workflow and topology.
 * @param selection - full or partial execution request.
 * @returns topologically ordered scheduled nodes plus explicit boundary inputs.
...
export function planMediaWorkflowExecution(
  ...
  return {
    ...
    selection,
    scheduledNodeIds: workflow.topologicalNodeIds.filter(nodeId => scheduled.has(nodeId)),
    targetNodeIds: roots,
