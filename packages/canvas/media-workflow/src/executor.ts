/** Provider-neutral exact-version executor registry and executor-result validation. */

import type { MediaNodeDefinition, MediaNodeDefinitionRef } from './types.ts'
import type {
  MediaNodeExecutor,
...
/** Stable runtime failure raised while dispatching or validating N12 execution. */
...
/** Open-world exact type/version executor table. Registration returns an idempotent disposer. */
export class MediaNodeExecutorRegistry {
   ...
   * @param executor - provider-neutral implementation.
  ...
  /**
   * Resolve one exact executor.
   * @param ref - exact semantic node type/version.
   * @returns registered executor.
   * @throws MediaWorkflowExecutionError when no executor is registered.
   */
...
function assertOutput(result: MediaNodeExecutorResult, definition: MediaNodeDefinition): void {
  for (const [name, output] of Object.entries(result.outputs)) {
    ...
    if (port === undefined) {
      throw new MediaWorkflowExecutionError(
        ...
        `Executor ${definition.type}@${definition.version} returned unknown output port ${name}`,
      )
    }
    if (output.value.kind !== port.type) {
      throw new MediaWorkflowExecutionError(
        'MEDIA_WORKFLOW_INVALID_EXECUTOR_OUTPUT',
        `Executor ${definition.type}@${definition.version} returned ${output.value.kind} for ${name}; expected ${port.type}`,
      )
    }
    if (output.fingerprint.trim() !== '') continue
    throw new MediaWorkflowExecutionError(
      'MEDIA_WORKFLOW_INVALID_EXECUTOR_OUTPUT',
      `Executor ${definition.type}@${definition.version} returned an empty fingerprint for ${name}`,
    )
  }
  for (const port of definition.outputs) {
    ...
    throw new MediaWorkflowExecutionError(
      'MEDIA_WORKFLOW_INVALID_EXECUTOR_OUTPUT',
      `Executor ${definition.type}@${definition.version} omitted required output ${port.name}`,
    )
  }
}
...
/**
 * Validate, detach, and recursively freeze one executor or cache result.
 * @param result - executor/cache-owned result object.
 * @param definition - exact node definition used to validate output names and value kinds.
 * @returns immutable detached result safe for downstream execution.
 */
export function snapshotMediaNodeExecutorResult(
  result: MediaNodeExecutorResult,
