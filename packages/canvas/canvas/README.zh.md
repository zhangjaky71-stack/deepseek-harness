...
`dsh-canvas` 负责会话范围内的媒体 Canvas 领域词汇：语义媒体工作流、彼此独立的 workflow/run revision、持久媒体引用、run/output 摘要，以及从这些值派生的展示状态。N01 有意保持无状态；Session 事件、projection、Remote、Agent 工具、Provider 和 UI 属于后续 Canvas 层。
...
单独发布的 `./invariant` companion 目前注册带说明的空运行时不变式，因为 N01 只拥有不可变值。等 Canvas service 层存在持久 Canvas 事件后，该层会加入 Session 事件／数据关系检查。
...
- **尚无持久 authority** — N01 只定义和校验 Canvas 值，不追加 Session 事件、不折叠回放、也不暴露 service；事件溯源 service 属于后续实施节点。
