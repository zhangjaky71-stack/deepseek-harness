...
`dsh-canvas` owns the session-scoped media Canvas domain vocabulary: semantic media workflows, independent workflow/run revisions, durable media references, run/output summaries, and presentation state derived from those values. N01 is intentionally state-free; Session events, projections, Remotes, Agent tools, providers, and UI belong to later Canvas layers.
...
The separately published `./invariant` companion currently registers an explained empty runtime invariant because N01 owns immutable values only. The Canvas service layer replaces that emptiness with Session event/data relationship checks when durable Canvas events exist.
...
Zero direct tokens. Domain construction and validation do not alter model requests.
...
- **No durable authority yet** — N01 defines and validates Canvas values but does not append Session events, fold replay, or expose a service; the event-sourced service is a later implementation node.
