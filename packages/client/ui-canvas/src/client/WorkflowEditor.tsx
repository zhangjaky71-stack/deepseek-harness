...
export function WorkflowEditor(props: WorkflowEditorProps) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      void commitCommand(commandFor(workflow, t('editor.commandEditNode'), operations), draft.baseWorkflowRevision).then(result => { if (result.ok) actions.markDraftClean(result.workflowRevision) })
    }, 450)
  ...
  return <div className={css.editor} tabIndex={0} onKeyDown={onKeyDown}>
    <aside className={css.leftRail}><NodeLibrary catalog={props.nodeCatalog} onAdd={definition => { void addDefinition(definition) }} t={t} /><div className={css.shortcutHelp}>{t('editor.shortcuts')}</div></aside>
    ...
    <aside className={css.rightRail}><NodeInspector node={selectedNode} draft={draft} saveStatus={saveStatus} onNameChange={actions.setDraftName} onConfigChange={actions.setDraftConfig} t={t} /><ConnectionPanel workflow={workflow} catalog={props.nodeCatalog} disabled={saveStatus === 'saving'} onConnect={(source, target) => { void connect(source, target) }} t={t} /><ValidationPanel draft={draft} workflowRevision={canvas.workflowRevision} saveStatus={saveStatus} t={t} /></aside>
  </div>
}
