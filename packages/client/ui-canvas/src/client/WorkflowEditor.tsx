...
export function WorkflowEditor(props: WorkflowEditorProps) {
  ...
  return <div className={css.editor} tabIndex={0} onKeyDown={onKeyDown}>
    ...
    <aside className={css.rightRail}><NodeInspector node={selectedNode} draft={draft} saveStatus={saveStatus} {...(selectedReadOnlyReason === undefined ? {} : { readOnlyReason: selectedReadOnlyReason })} onNameChange={actions.setDraftName} onConfigChange={actions.setDraftConfig} onBlur={() => { if (draft !== null) void saveDraft(draft) }} t={t} /><ConnectionPanel workflow={workflow} catalog={props.nodeCatalog} capabilities={props.capabilities} disabled={saveStatus === 'saving'} onConnect={(source, target) => { void connect(source, target) }} t={t} /><ValidationPanel draft={draft} workflowRevision={canvas.workflowRevision} saveStatus={saveStatus} t={t} /></aside>
  </div>
}
