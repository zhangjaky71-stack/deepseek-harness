...
export const muxFrameSchema = z.discriminatedUnion('type', [
  ...
  // `-1` is the framework's empty-log watermark, shared with subscribed/history baselines.
  // `value` stays wide: domain values and the reserved projection visibility control
  // envelope have already been validated by the Host projection registry.
  z.object({ type: z.literal('session/projection'), sessionId: sessionIdSchema, key: z.string().min(1), value: z.unknown(), seq: z.number().int().min(-1) }),
